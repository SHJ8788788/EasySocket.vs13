...
namespace EasySocket.vs13.Telegram.Easy.Internal
{
    ...
    internal static class Common
    {
        ...
        /// <returns></returns>
        public static bool SendRemoteData(ActionContext actionContext)
        {
            try
            {
                var packet = actionContext.Packet;
                actionContext.Session.UnWrap().SendData(packet.ToArraySegment());
                return true;
            }
            catch (Exception)
            {
        ...
        public static bool SendRemoteException(IWrapper sessionWrapper, ExceptionContext exceptionContext)
        {
            try
            {
                ...
                sessionWrapper.UnWrap().SendData(packet.ToArraySegment());
                return true;
            }
            catch (Exception)
            {
        ...
        /// <returns></returns>
        public static ApiResult<T> InvokeApi<T>(ISession session, TaskSetterTable<long> taskSetActionTable, ISerializer serializer, EasyPacket packet, TimeSpan timeout)
        {
            var taskSetter = taskSetActionTable.Create<T>(packet.Id).TimeoutAfter(timeout);
            //判断session是否已连接
            if (session.IsConnected == false)
            {
                var taskSetAction = taskSetActionTable.Take(packet.Id);
                taskSetAction.SetException(new RemoteException("与服务器的连接已断开！！"));
                return new ApiResult<T>(taskSetter);
            }
            session.SendData(packet.ToArraySegment());
            return new ApiResult<T>(taskSetter);
        }
