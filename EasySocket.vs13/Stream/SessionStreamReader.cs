    ...
    public class SessionStreamReader : ISessionStreamReader
    {
        ...
        public byte this[int index]
        {
            get
            {
                ...
                //return this.Stream.ToArray()[index];
                return this.Stream.ToArray()[index];
            }
        }
        ...
        {
            var range = new ArraySegment<byte>(this.Stream.ToArray(), this.Position, count);
        ...
        /// </summary>
        /// <param name="count">清除的字节数</param>
        public void ClearFromEnd(int count)
        {
            if (count < 0 || count > this.Length)
            {
                throw new ArgumentOutOfRangeException();
            ...
            if (length > 0)
            {
                this.CopyTo(0, this.Stream.GetBuffer(), 0, length);
            }
        ...
        /// <summary>
        /// 清除此次接收的数据，并还原为接收前的状态
        /// </summary>
        public void Restore()
        {
            ClearFromEnd(this.ReceiveLength);
            ReceiveLength = 0;
        }
        ...
        public void CopyTo(int srcOffset, byte[] dstArray, int dstOffset, int count)
        {
            ...
            //byte[] dstA = new byte[256];
            //Buffer.BlockCopy(this.Stream.GetBuffer(), srcOffset, dstA, dstOffset, count);
        }
        ...
        public unsafe int IndexOf(byte[] bin)
        {
            ...
            fixed (byte* pBytes = &this.Stream.ToArray()[this.Position], pBin = &bin[0])
            {
        ...
        {
            fixed (byte* pBytes = &this.Stream.ToArray()[this.Position])
            {
